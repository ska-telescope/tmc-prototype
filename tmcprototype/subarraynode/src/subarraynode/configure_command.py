...
class Configure(SKASubarray.ConfigureCommand):
    ...
    def _configure_csp(self, scan_configuration):
        device_data = DeviceData.get_instance()
        attr_name_map = {
            const.STR_DELAY_MODEL_SUB_POINT: device_data.csp_subarray_ln_fqdn
            + "/delayModel",
        }
        cmd_data = self._create_cmd_data(
            ...
            scan_configuration,
            attr_name_map,
            device_data._receive_addresses_map,
        )
    ...
    def _configure_dsh(self, scan_configuration):
        ...
        cmd_data = self._create_cmd_data("build_up_dsh_cmd_data", scan_configuration)

        try:
            device_data._dish_leaf_node_group_client.send_command(
                const.CMD_CONFIGURE, cmd_data
            )
            self.logger.info(
                "Configure command is invoked on the Dish Leaf Nodes Group"
            )
            device_data._dish_leaf_node_group_client.send_command(
...
class ElementDeviceData:
    ...
    def build_up_sdp_cmd_data(scan_config):
        ...
        if sdp_scan_config:
            ...
            if not scan_type:
                raise KeyError(
                    "SDP Subarray scan_type is empty. Command data not built up"
                )
        else:
            ...
            raise KeyError(
                "SDP configuration must be given. Aborting SDP configuration."
            )
        ...
        if csp_scan_config:
            ...
            if scan_type:
                ...
                if receive_addresses_map:
                    csp_config_schema = interface.make_csp_config(
                    ...
                    raise KeyError(
                        "Receive addresses must be given. Aborting CSP configuration."
                    )
            else:
            ...
            if csp_config_schema:
                ...
                raise KeyError(
                    "CSP configuration schema must be given. Aborting CSP configuration."
                )

        else:
            raise KeyError(
                "CSP configuration must be given. Aborting CSP configuration."
            )
    ...
    def build_up_dsh_cmd_data(scan_config):
        ...
        if set(["pointing", "dish"]).issubset(scan_config.keys()):
            ...
            raise KeyError(
                "Dish configuration must be given. Aborting Dish configuration."
            )
        return cmd_data
