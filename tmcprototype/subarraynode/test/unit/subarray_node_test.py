...
@pytest.fixture(scope="function")
def csp_func_args():
    attr_name_map = {"string1": "attr1", "string2": "attr2"}
...
class TestElementDeviceData:
    def test_build_up_sdp_cmd_data_with_valid_scan_configuration(self):
        ...
        expected_string_dict = {"scan_type": "science_A"}
    ...
    def test_build_up_sdp_cmd_data_with_scan_type_missing_configuration(
        self, example_invalid_scan_configuration
    ):
    ...
    def test_build_up_csp_cmd_data_with_valid_scan_configuration(
        self, csp_func_args, sdp_func_receive_addresses
    ):
        valid_scan_config = json.loads(configure_str)
        attr_name_map = csp_func_args
        receive_addresses_map = sdp_func_receive_addresses
        csp_cmd_data = ElementDeviceData.build_up_csp_cmd_data(
            valid_scan_config, attr_name_map, receive_addresses_map
        )
    ...
    def test_build_up_csp_cmd_data_with_empty_receive_addresses(self, csp_func_args):
        ...
        with pytest.raises(KeyError) as exception:
            ElementDeviceData.build_up_csp_cmd_data(
                valid_scan_config, attr_name_map, receive_addresses_map
            )
        expected_msg = "Receive addresses must be given. Aborting CSP configuration."
        assert exception.value.args[0] == expected_msg

    def test_build_up_csp_cmd_data_with_empty_scan_configuration(
        self, csp_func_args, sdp_func_receive_addresses
    ):
        empty_scan_config = {}
        attr_name_map = csp_func_args
        ...
        with pytest.raises(KeyError) as exception:
            ElementDeviceData.build_up_csp_cmd_data(
                empty_scan_config, attr_name_map, receive_addresses_map
            )
        expected_msg = "CSP configuration must be given. Aborting CSP configuration."
        assert exception.value.args[0] == expected_msg

    def test_build_up_csp_cmd_data_with_invalid_scan_configuration(
        self, csp_func_args, sdp_func_receive_addresses
    ):
        ...
        with pytest.raises(KeyError) as exception:
            ElementDeviceData.build_up_csp_cmd_data(
                invalid_scan_config, attr_name_map, receive_addresses_map
            )
        expected_msg = "CSP configuration must be given. Aborting CSP configuration."
        assert exception.value.args[0] == expected_msg
...
def test_assign_resource_should_command_dish_csp_sdp_subarray1_to_assign_valid_resources(
    ...
    verify_called_correctly(
        tango_client_obj.deviceproxy, const.CMD_ASSIGN_RESOURCES, str_json_arg
    )
...
def assert_that_log_contains(name: str, caplog):
    patterns = [
        f"^Transaction.*(?<=Enter\[{name}\])",
        f"^Transaction.*(?<=Exit\[{name}\])",
    ]
    for pattern in patterns:
        ...
        if not found:
            raise AssertionError(
                f"pattern ({pattern}) not found in expected log messages"
            )
...
def test_log_transaction_with_config(mock_device_proxy, subarray_state_model, caplog):
    ...
    with mock.patch.object(TangoClient, "_get_deviceproxy", return_value=Mock()):
        with mock.patch.object(
            TangoClient,
            "subscribe_attribute",
            side_effect=dummy_subscriber_receive_addresses,
        ):
...
def test_transaction_id_injected_in_config_command(
    mock_device_proxy, subarray_state_model, mock_transaction_id
):
    device_proxy, tango_client_obj = mock_device_proxy
    device_data = DeviceData.get_instance()
    configure_cmd = Configure(device_data, subarray_state_model)
    with mock.patch.object(TangoClient, "_get_deviceproxy", return_value=Mock()):
        with mock.patch.object(
            ...
            configure_cmd.do(configure_str)
            verify_called_correctly(
                tango_client_obj.deviceproxy, const.CMD_CONFIGURE, mock_transaction_id
            )


def test_transaction_id_injected_in_assign_command(
    mock_device_proxy, mock_transaction_id
):
    device_proxy, tango_client_obj = mock_device_proxy
    device_proxy.On()
    device_proxy.AssignResources(assign_input_str)
    verify_called_correctly(
        tango_client_obj.deviceproxy, const.CMD_ASSIGN_RESOURCES, mock_transaction_id
    )
...
def test_configure_command_subarray_with_invalid_configure_input(
    device_data, subarray_state_model, mock_device_proxy
):
    device_proxy, tango_client_obj = mock_device_proxy
    configure_cmd = Configure(device_data, subarray_state_model)
    with mock.patch.object(TangoClient, "_get_deviceproxy", return_value=Mock()):
...
def test_scan_command(device_data, subarray_state_model, mock_device_proxy):
    ...
    assert scan_cmd.do(configure_str) == (
        ResultCode.STARTED,
...
def test_releaseallresources_command(
    device_data, subarray_state_model, mock_device_proxy
):
    device_proxy, _ = mock_device_proxy
    device_proxy.On()
...
def test_subarray_health_state_when_csubarray1_ln_is_in_health_state_after_start(
    mock_device_proxy, csp_health_state
):
    device_proxy, tango_client_obj = mock_device_proxy
    device_data = DeviceData.get_instance()
    with mock.patch.object(
...
def create_dummy_event_sdp_receiceAddresses(
    proxy_mock, device_fqdn, attribute, attr_value
):
    fake_event = Mock()
    fake_event.err = False
    fake_event.attr_name = f"{device_fqdn}/{attribute}"
...
def fake_tango_system(
    ...
    device_group_import_path="tango.Group",
):
    with mock.patch(device_proxy_import_path) as patched_constructor:
        with mock.patch(device_group_import_path) as group_constructor:
            ...
            group_constructor.side_effect = lambda dish_group_mock: group_to_mock.get(
