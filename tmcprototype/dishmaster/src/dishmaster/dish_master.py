...
# class DishMaster(SKAMaster):
    ...
    def init_device(self):
        ...
        try:
            ...
            tango.Except.throw_exception(const.STR_CMD_FAILED, log_msg,
                                         const.ERR_CONFIG_DM, tango.ErrSeverity.ERR)
        ...
        try:
            ...
            if(float(actual_az_lim) <= self.preconfig_az_lim and
               float(actual_el_lim) <= self.preconfig_el_lim) is True:
                #if dish is within the preconfigured limit then dish will Track.
                # Check if stoptrack command is invoked on Dish, if not then only set the Pointingstate to TRACK
    ...
    def Configure(self, argin):
        ...
        try:
            ...
            self.logger.error(log_msg)
            tango.Except.throw_exception(const.STR_CMD_FAILED, log_msg,
                                         const.ERR_JSON_KEY_NOT_FOUND, tango.ErrSeverity.ERR)
    ...
    def Abort(self):
        ...
        try:
            if (self._pointing_state == PointingState.SLEW):
                ...
                log_msg = "Abort -> IN SLEW: " + str(self._abort_in_slew)
                self.logger.error(log_msg)
                self._pointing_state = PointingState.READY
            else:
                self._pointing_state = PointingState.READY
    ...
    def Restart(self):
        ...
        try:
            ...
            log_msg = "Restart -> IN SLEW: " + str(self._abort_in_slew)
            self.logger.error(log_msg)
