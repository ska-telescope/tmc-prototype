...
class SubarrayNode(with_metaclass(DeviceMeta, SKASubarray)):
    ...
    def Configure(self, argin):
        ...
        try:
            ...
            [exception_message, exception_count] = self._handle_devfailed_exception(dev_failed,
                                            exception_message, exception_count, CONST.ERR_CONFIGURE_CMD_GROUP)
        except Exception as except_occurred:
            [exception_message, exception_count] = self._handle_generic_exception(except_occurred,
                                                exception_message, exception_count, CONST.ERR_CONFIGURE_CMD)
        # Throw Exception
        if exception_count > 0:
    ...
    def init_db(self):
        ...
        connection = mysql.connector.connect(
            host=CONST.MYSQL_HOSTNAME,
            user=CONST.MYSQL_USER,
            passwd=CONST.MYSQL_PASSWORD,
            database=CONST.MYSQL_DB_NAME,
