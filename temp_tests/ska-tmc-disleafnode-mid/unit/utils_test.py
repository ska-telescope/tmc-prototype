...
from src.ska_tmc_dishleafnode_mid.utils import UnitConverter
...
class TestUnitConverter:
    ...
    def test_dms_to_rad(self):
        ...
        expected_output = (
            float(ref_input[0])
            + (float(ref_input[1]) / 60)
            + (float(ref_input[2]) / 3600)
        ) * (math.pi / 180)
    ...
    def test_rad_to_dms(self):
        ...
        expected_output.append(
            (
                ...
                - int((ref_input_dd - int(ref_input_dd)) * 60)
            )
            * 60
        )
    ...
    def test_dms_to_dd(self):
        ...
        expected_output = (
            float(ref_input_list[0])
            + (float(ref_input_list[1]) / 60)
