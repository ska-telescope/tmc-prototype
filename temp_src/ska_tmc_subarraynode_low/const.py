...
STR_CONFIGURE_CMD_INVOKED_SA_LOW = (
    "Configure command invoked on SubarrayNode Low."
)
...
SCAN_NOT_EXECUTED = (
    "Scan can not be executed as Subarray.obsState is not READY."
)
RESOURCE_ALREADY_RELEASED = "Resources are already released from Subarray"
STR_MCCS_END_SCAN_INIT = "MCCS EndScan is initiated."
STR_CMD_ABORT_INV_MCCS = (
    "Abort command invoked successfully on MccsSubarrayLeafNode."
)
STR_CMD_RESTART_INV_MCCS = (
    "Restart command is invoked on MccsSubarrayLeafNode."
)
...
STR_CMD_OBSRESET_INV_MCCSSLN = (
    "Command ObsReset is invoked on MccsSubarrayLeafNode."
)
STR_RESTART_SUCCESS = (
    "Restart command invoked successfully on MccsSubarrayLeafNode."
)
...
ERR_MCCS_SUBARRAY_OBS_STATE = (
    "Key Error occurred while setting MCCS Subarray obsState"
)
...
ERR_RESTART_INVOKING_CMD = (
    "Error while invoking Restart command on Subarray Node."
)
ERR_END_SCAN_CMD_ON_MCCS = (
    "Error invoking EndScan command on MccsSubarrayLeafNode"
)
...
ERR_SUBSR_MCCSSA_ASSIGNED_RES_ATTR = (
    "Error in subscribing assigned_resources attribute "
)
