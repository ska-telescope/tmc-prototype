...
class ConsumeImmediately(interfaceStrategy):
    ...
    def subscribe(self, attr):
        ...
        """
        self.listening = True
        self.current_subscriptions.append(
            {
                "subscription": self.device_proxy.subscribe_event(
                    attr, EventType.CHANGE_EVENT, self._cb
                ),
                "attr": attr,
            }
        )
    ...
    async def async_subscribe(self, attr):
        ...
        self.listening = True
        self.current_subscriptions.append(
            await self.device_proxy.subscribe_event(
                attr, EventType.CHANGE_EVENT, self._cb
            )
        )
        ...
        if self.listening:
            try:
                ...
                raise ListenerTimeOut(
                    f"Timed out after {timeout} seconds waiting for events on attributes"
    ...
    def unsubscribe(self, *attrs):
        ...
        if attrs == ():
            for current_subscription in self.current_subscriptions:
                self.device_proxy.unsubscribe_event(
...
class ConsumePeriodically(interfaceStrategy):
    ...
    def subscribe(self, attr):
        ...
        current_subscription = {
            "subscription": self.device_proxy.subscribe_event(
                attr, EventType.CHANGE_EVENT, self.buffer_size
            ),
...
class Listener:
    ...
    def _remember_polling(self, attr):
        ...
        if self.device_proxy.is_attribute_polled(attr):
            self.original_server_polling = self.device_proxy.get_attribute_poll_period(
                attr
            )
    ...
    def _restore_polling(self):
        if self.original_server_polling is not None:
            self.device_proxy.poll_attribute(
                self.attribute_being_monitored, self.original_server_polling
            )
        else:
    ...
    async def async_get_events_on(
        self, attr, max_events=None, get_elapsed_time=False, timeout=5
    ):
        ...
        while self.listening:
            events, elapsed_time = await self.async_wait_for_next_event(
                timeout, get_elapsed_time=True
            )
            for event in events:
    ...
    def get_events_on(self, attr, max_events=None, get_elapsed_time=False, timeout=5):
        ...
        while self.listening:
            events, elapsed_time = self.wait_for_next_event(
                timeout, get_elapsed_time=True
            )
            for event in events:
                if get_elapsed_time:
    ...
    def _time_events(self, timeout, resolution):
        for the_listener, binding in self.listeners.items():
            for attr in binding["attrs"].values():
                ...
                context = {"handler": the_handler, "listener": the_listener}
            ...
            for the_listener, binding in self.listeners.items():
                ...
                if events is not None:
                    ...
                    for event in events:
                        ...
                        the_tracer.message(
                            f"yielding event {event} for {attr}"
                            f" to be handled by {the_handler}"
                            f" at {datetime.now()}"
                        )
                        yield HandeableEvent(
                            the_handler, event, timer.time, the_listener
                        )
