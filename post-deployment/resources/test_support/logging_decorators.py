...
def print_logs_to_file(s, d, log_name, status="ok"):
    if status == "ok":
        filename_d = "logs_test_{}_{}.csv".format(
            log_name, datetime.now().strftime("%d_%m_%Y-%H_%M")
        )
        filename_s = "states_test_{}_{}.csv".format(
            log_name, datetime.now().strftime("%d_%m_%Y-%H_%M")
        )
    elif status == "error":
        filename_d = "error_logs_test_{}_{}.csv".format(
            log_name, datetime.now().strftime("%d_%m_%Y-%H_%M")
        )
        filename_s = "error_states_test_{}_{}.csv".format(
            log_name, datetime.now().strftime("%d_%m_%Y-%H_%M")
        )
    LOGGER.info(
        "Printing log files to build/{} and build/{}".format(filename_d, filename_s)
    )
...
def log_it(log_name, devices_to_log, non_default_states_to_check):
    def decorator_log_it(func):
        if local_elastic_disabled:
            ...
            def wrapper(*args, **kwargs):
                s = StateChecker(
                    devices_to_log, specific_states=non_default_states_to_check
                )
...
def print_states_logs_to_file(s, log_name, status="ok"):
    if status == "ok":
        ...
        raise Exception(
            f'Incorrect status flag: status must be "ok" or "error" but {status} was given'
        )
...
@contextmanager
def log_states(
    log_name, devices_to_log, state_name="obsState", non_default_states_to_check={}
):
    s = StateChecker(
        devices_to_log,
