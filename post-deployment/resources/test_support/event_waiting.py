import logging
from resources.test_support.subscribing import (
    EventData,
    MessageBoard,
    MessageHandler,
    Tuple,
...
class WaitUntilEqual(MessageHandler):
    ...
    def update(self, event: EventData) -> None:
        ...
        self.tracer.message(
            f"new event added to list, current list size is {len(self.events)}"
        )

    def handle_event(
        self, event: EventData, subscription: Subscription, logger: Logger
    ) -> None:
...
class WaitUntilChanged(MessageHandler):
    ...
    def describe_self(self) -> str:
        return (
            f"handler that waits for any change on {self.attr} on device {self.device}"
        )
    ...
    def update(self, event: EventData):
        ...
        if self.starting_value is None:
            ...
            self.tracer.message(
                f"first event received, starting state = {self.starting_value}"
            )
...
class WaitForOrderedChange(WaitUntilEqual):
    ...
    def _is_partialy_correct(self) -> bool:
        ...
        if not current_event.err:
            ...
            if current_value == expected_value:
                ...
                self.tracer.message(
                    f"WARNING: current event has a value:{current_value} different than what is expected: {expected_value}"
                )
        return False
    ...
    def condition_met(self) -> bool:
        ...
        if self._is_partialy_correct():
            ...
            self.tracer.message(
                f"current event {self._current_event()} is correct, waiting for {self._expected_event()}"
            )
...
def set_wating_for_start_up() -> MessageBoardBuilder:
    ...
    b.set_waiting_on("mid_d0001/elt/master").for_attribute("State").to_become_equal_to(
        "ON"
    )
    b.set_waiting_on("mid_d0002/elt/master").for_attribute("State").to_become_equal_to(
        "ON"
    )
    b.set_waiting_on("mid_d0003/elt/master").for_attribute("State").to_become_equal_to(
        "ON"
    )
    ...
    b.set_waiting_on("ska_mid/tm_central/central_node").for_attribute(
        "State"
    ...
    b.set_waiting_on("mid_sdp/elt/subarray_1").for_attribute(
        "State"
    ).to_become_equal_to("ON")
    return b
...
def set_waiting_for_assign_resources(id: int) -> MessageBoard:
    b = MessageBoardBuilder()
    b.set_waiting_on(f"ska_mid/tm_subarray_node/{id}").for_attribute(
        "obsState"
    ).to_become_equal_to("ObsState.IDLE", master=True)
    b.set_waiting_on(f"mid_csp/elt/subarray_{id:02d}").for_attribute(
        "obsState"
    ).and_observe()
    b.set_waiting_on(f"mid_sdp/elt/subarray_{id}").for_attribute(
        "obsState"
    ).and_observe()
    for dishnr in range(1, 3):
        # b.set_waiting_on(f'ska_mid/tm_leaf_node/d{dishnr:04d}').for_attribute('dishPointingState').and_observe()
        b.set_waiting_on(f"mid_d{dishnr:04d}/elt/master").for_attribute(
            "pointingState"
        ).and_observe()
    return b
...
def set_waiting_for_release_resources(id: int) -> MessageBoard:
    ...
    b.set_waiting_on(f"ska_mid/tm_subarray_node/{id}").for_attribute(
        "obsState"
    ).to_become_equal_to("ObsState.EMPTY", master=True)
    b.set_waiting_on(f"mid_csp/elt/subarray_{id:02d}").for_attribute(
        "obsState"
    ).and_observe()
    b.set_waiting_on(f"mid_sdp/elt/subarray_{id}").for_attribute(
...
def set_waiting_for_configure_scan(id: int) -> MessageBoard:
    b = MessageBoardBuilder()
    subarray_change_order = ["ObsState.CONFIGURING", "ObsState.READY"]
    b.set_waiting_on(f"ska_mid/tm_subarray_node/{id}").for_attribute(
        "obsState"
    ).to_change_in_order(subarray_change_order, master=True)
    b.set_waiting_on(f"mid_csp/elt/subarray_{id:02d}").for_attribute(
        "obsState"
    ).and_observe()
    b.set_waiting_on(f"mid_sdp/elt/subarray_{id}").for_attribute(
...
def set_waiting_for_scanning_to_complete(id: int) -> MessageBoard:
    ...
    b.set_waiting_on(f"ska_mid/tm_subarray_node/{id}").for_attribute(
        "obsState"
    ).to_change_in_order(["ObsState.SCANNING", "ObsState.READY"], master=True)
    b.set_waiting_on(f"mid_csp/elt/subarray_{id:02d}").for_attribute(
        "obsState"
    ).and_observe()
    b.set_waiting_on(f"mid_sdp/elt/subarray_{id}").for_attribute(
...
def try_getting_wait_result(
    board: MessageBoard, future: futures.Future, timeout: int, silent: bool = False
) -> None:
    try:
...
def sync_subarray_assigning(id, logger, timeout=10, log_enabled=False):
    ...
    try:
        ...
        board.log(
            "Subarray assignment call resulted in errors,...getting results from waiting on its state"
        )
    ...
    try:
        board.log(
            "Subarray assignment call returned successfully...getting results from waiting on its state"
        )
...
def sync_subarray_releasing(id: int, logger, timeout=10, log_enabled=False):
    ...
    try:
        ...
        board.log(
            "Subarray release call resulted in errors,...getting results from waiting on its state"
        )
        try_getting_wait_result(board, future_wait_result, timeout, silent=True)
        print_when_exception(board, logger)
        raise e
    try:
        board.log(
            "Subarray release call returned successfully...getting results from waiting on its state"
        )
        try_getting_wait_result(board, future_wait_result, timeout)
    except Exception as e:
        print_when_exception(board, logger)
...
# configure a scan
@contextmanager
def sync_subarray_configuring(
    id: int, logger: logging.Logger, timeout: int = 10, log_enabled: bool = False
):
    ...
    try:
        ...
        board.log(
            "Subarray configure call resulted in errors,...getting results from waiting on its state"
        )
        try_getting_wait_result(board, future_wait_result, timeout, silent=True)
        print_when_exception(board, logger)
        raise e
    try:
        board.log(
            "Subarray configure call returned successfully...getting results from waiting on its state"
        )
        try_getting_wait_result(board, future_wait_result, timeout)
    except Exception as e:
        print_when_exception(board, logger)
...
@contextmanager
def sync_release_configuration(
    id: int, logger: logging.Logger, timeout: int = 10, log_enabled: bool = False
):
    ...
    try:
        ...
        board.log(
            "Subarray configure tear down resulted in an error,...getting results from waiting on its state"
        )
        try_getting_wait_result(board, future_wait_result, timeout, silent=True)
        print_when_exception(board, logger)
        raise e
    try:
        board.log(
            "Subarray configure tear down returned successfully...getting results from waiting on its state"
        )
        try_getting_wait_result(board, future_wait_result, timeout)
    except Exception as e:
        print_when_exception(board, logger)
...
def observe_states(specs: List[Tuple[str, str]], logger, timeout=10, log_enabled=True):
    b = MessageBoardBuilder()
    for spec in specs:
        b.set_waiting_on(spec.device).for_attribute(
            spec.attr, polling=False
        ).and_observe()
    board = b.setup_board()
    try:
        yield
