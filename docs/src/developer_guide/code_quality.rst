...
Black
^^^^^
TMC SDP Leaf Nodes uses the ``black`` code formatter to format its code. 
Formatting can be done using the command ``make python-format``.

The CI pipeline does check that if code has been formatted using black or not.

Linting
^^^^^^^
TMC SDP Leaf Nodes uses below libraries/utilities for linting. 
