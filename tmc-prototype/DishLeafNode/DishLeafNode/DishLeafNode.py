...
class DishLeafNode(SKABaseDevice):
    ...
    def init_device(self):
        ...
        try:
            ...
            self.set_status("Dish Leaf Node initialized successfully.")
            self.devlogmsg("Dish Leaf Node initialized successfully.", 4)

        except Exception as e:
            print "Exception occurred while subscribing to Dish attributes", e
            self._read_activity_message = "Exception occurred while subscribing to Dish attributes" + str(e)
            self.set_state(DevState.FAULT)
